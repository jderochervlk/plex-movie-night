...
module Data = {
  let make = async (~ratingKey, ~wantToWatch, ~user) => {
    switch await Plex.getMetadata(ratingKey) {
    | Some(mediaContainer) => Some({mediaContainer, wantToWatch, user})
    | None => None
    }
  }
}
...
let handler: Fresh.Handler.t<unknown, data, unknown> = {
  get: async (req, ctx) => {
    ...
    | (None, Some(ratingKey)) => {
        ...
        let data = await Data.make(~ratingKey, ~wantToWatch, ~user)
...
let make = (~data: option<data>) => {
  ...
  | Some(Movie({title, summary, thumb, ratingKey})) =>
    <Movie title summary thumb ratingKey wantToWatch name />
  | _ =>
